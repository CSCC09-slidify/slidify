    ...
    <v-menu class="notification-list" :activator="activator" v-model="isOpened">
        <v-card class="mx-auto" max-width="450" color="white">
            ...
            <v-list v-if="content.length > 0">
                ...
                >
                    <v-list-item-title
                        :style="{ lineHeight: 1.3, marginBottom: '5px', whiteSpace: 'normal' }"
                    >
                        {{ item.text }}
                    </v-list-item-title>
            ...
            <div class="d-flex justify-center pa-4" v-if="content.length > 0">
                <v-btn @click="clearNotifications" color="primary" class="w-100" :disabled="content.length == 0">
                    Clear Notifications
                </v-btn>
            </div>
            <v-card-subtitle class="py-4" v-if="content.length == 0">You have no notifications</v-card-subtitle>
        </v-card>
    </v-menu>
</template>
<script>
